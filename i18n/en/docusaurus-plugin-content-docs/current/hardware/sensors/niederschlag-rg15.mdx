...
This adapter board is included in the sensor package and is connected to the senseBox using the {useBoardStore((state) => (state.board === 'MCU-S2' || state.board === ':edu S2') ? <a href="https://sensebox.kaufen/product/qwiic-kabel-500mm">QWIIC cable SB-074</a> : <a href="https://sensebox.kaufen/product/sensebox-kabel-jst-jst-500mm">senseBox cable SB-057</a>)}.
...
To program the sensor in Arduino, you need to install the software library [RG15-Arduino](https://github.com/sensebox/rg15-arduino-lib). A detailed guide on how to add libraries can be found in our [documentation](https://docs.sensebox.de/en/docs/editors/arduino/libraries-hinzufuegen/).
...
{useBoardStore((state) => (state.board === 'MCU-S2' || state.board === ':edu S2') ? (
	<>
		<p>In this example, the sensor is initialized via Serial0, which is the default port on the senseBox MCU S2 or senseBox:edu S2.</p>
...
void loop(){
	...
	<>
		<p>In this example, the sensor is initialized via Serial1. On the senseBox MCU or senseBox:edu, Serial2 can also be used as an alternative.</p>
		<pre><code className="language-cpp">{`#include <RG15.h> // https://github.com/sensebox/rg15-arduino-lib
...
## Extras
> - [Shop](https://sensebox.kaufen/products/hydreon-rg-15-regensensor)
