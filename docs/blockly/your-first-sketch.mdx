...
{/* /// access board store with useBoardStore((state)=>state.board) */}

<Layout title="Blockly">
...
## Erste Schritte 
{ useBoardStore((state)=>state.board) === 'senseBoxMCU' ? 'Hier siehst du nur die MCU' : 'Hier siehst du nur die senseBox' }
