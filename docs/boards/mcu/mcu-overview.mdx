...
const toggleTopTab = (tab) => {
  if(tab === 'Pinout' || tab === 'DataSheet') {
    window.open(tab === 'DataSheet' ? 'https://learn.watterott.com/de/sensebox/mcu/' : 'https://github.com/watterott/senseBox-MCU/blob/master/hardware/senseBox-MCU_v15.pdf', '_blank');
    return;
  } else {  
    if (activeTopTab === tab) {
